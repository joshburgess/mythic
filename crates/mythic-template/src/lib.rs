...
const KATEX_AUTO_RENDER_JS: &str =
...
fn compute_pluralize(text: &str) -> String {
    if text.ends_with('s') || text.ends_with("sh") || text.ends_with("ch") || text.ends_with('x') {
        format!("{text}es")
...
fn compute_singularize(text: &str) -> String {
    if let Some(stripped) = text.strip_suffix("ies") {
        format!("{stripped}y")
...
impl TemplateEngine {
    ...
    pub fn new_with_default(template_dir: &Path, default_engine: &str) -> Result<Self> {
        ...
        // Also load .tera, .jinja, .j2, .jinja2 files by reading them manually
                ...
                match ext {
                    ...
                    "jinja" | "j2" | "jinja2" => {
                        ...
                        mj.add_template_owned(rel.clone(), content)
                            .with_context(|| {
                                format!("Failed to load MiniJinja template: {}", path.display())
                            })?;
                        let layout_name = rel
        ...
        mj.add_filter(
            "reading_time",
            |value: String| -> Result<String, minijinja::Error> {
                Ok(compute_reading_time(&value))
            },
        );
        mj.add_filter(
            "word_count",
            |value: String| -> Result<usize, minijinja::Error> { Ok(compute_word_count(&value)) },
        );
        mj.add_filter(
            "truncate_words",
        ...
        mj.add_filter(
            "markdownify",
            |value: String| -> Result<String, minijinja::Error> { Ok(compute_markdownify(&value)) },
        );
        mj.add_filter(
            "plainify",
            |value: String| -> Result<String, minijinja::Error> { Ok(compute_plainify(&value)) },
        );
        mj.add_filter(
            "humanize",
            |value: String| -> Result<String, minijinja::Error> { Ok(compute_humanize(&value)) },
        );
        mj.add_filter(
            "pluralize",
            |value: String| -> Result<String, minijinja::Error> { Ok(compute_pluralize(&value)) },
        );
        mj.add_filter(
            "singularize",
            |value: String| -> Result<String, minijinja::Error> { Ok(compute_singularize(&value)) },
        );
        mj.add_filter(
            "urlize",
            |value: String| -> Result<String, minijinja::Error> { Ok(compute_urlize(&value)) },
        );
        mj.add_filter(
            "safeHTML",
    ...
    pub fn render_with_base_context(
        ...
        match engine {
            ...
            "minijinja" | "jinja" => {
                // MiniJinja doesn't use tera::Context -- fall back to full render
                self.render_minijinja_from_base(page, base_ctx, layout)
            }
    ...
    fn render_minijinja(
        ...
        if let serde_json::Value::Object(ref mut map) = page_ctx {
            ...
            map.insert("has_math".to_string(), serde_json::Value::Bool(has_math));
        }
        data.insert("page".to_string(), page_ctx);
        data.insert(
            "content".to_string(),
        ...
        data.insert(
            "katex_css".to_string(),
            serde_json::Value::String(KATEX_CSS.to_string()),
        );
        data.insert(
            "katex_js".to_string(),
            serde_json::Value::String(KATEX_JS.to_string()),
        );
        data.insert(
            "katex_auto_render_js".to_string(),
            serde_json::Value::String(KATEX_AUTO_RENDER_JS.to_string()),
        );
    ...
    fn render_minijinja_from_base(
        &self,
        page: &Page,
        base_ctx: &tera::Context,
        layout: &str,
    ) -> Result<String> {
        // MiniJinja doesn't use tera::Context -- extract base_path and fall back
        let bp = base_ctx
            .get("site")
            .and_then(|s| s.get("base_path"))
            .and_then(|v| v.as_str())
            .unwrap_or("");
        let base_url = base_ctx
            .get("site")
        ...
        // Copy assets and data from base context if present
        if let Some(assets) = base_ctx.get("assets") {
            data.insert(
                "assets".to_string(),
                serde_json::from_value(assets.clone()).unwrap_or_default(),
            );
        }
        if let Some(d) = base_ctx.get("data") {
            data.insert(
                "data".to_string(),
...
fn word_count_filter(
    ...
    Ok(tera::Value::Number(serde_json::Number::from(
        compute_word_count(&text),
    )))
}
...
mod tests {
    ...
    fn minijinja_syntax_error_returns_error() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("broken.jinja"), "{% if %}<p>bad syntax</p>").unwrap();
    ...
    fn minijinja_has_math_and_katex() {
        ...
        page.rendered_html =
            Some("<p>Inline math: <span class=\"math math-inline\">x^2</span></p>".to_string());
        let config = test_config();
        let html = engine.render(&page, &config).unwrap();
